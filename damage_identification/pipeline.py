...
class TestClu(Clustering):
    def __init__(self, params: Dict[str, Any]):
        super(TestClu, self).__init__("testclusterer", params)

    def train(self, examples: pd.DataFrame):
        pass

    def predict(self, example: pd.DataFrame) -> DamageMode:
        return DamageMode.DELAMINATION
...
class Pipeline:
    ...
    def __init__(self, params: Dict[str, Any]):
        ...
        self.clusterers: List[Clustering] = [TestClu(params)]
    ...
    def _predict(self, features):
        ...
        for clusterer in self.clusterers:
            predictions[clusterer.name] = features.apply(
                lambda series: clusterer.predict(series.to_frame().transpose()), axis=1
            )
